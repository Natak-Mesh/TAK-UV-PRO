...
/**
 * Serial BLE availability probes for MeshCore devices.
 * <ul>
...
final class BleMeshAvailabilityProber {
    ...
    private static final String TAG = "UVPro.MeshBLEProbe";
    ...
    private void enqueueProbe(Context context, Handler ioHandler, BluetoothDevice device,
        ...
        if (device == null || callback == null || ioHandler == null) {
            if (callback != null) callback.onResult(BUSY);
            return;
        }
    ...
    private void scheduleNext(Handler handler) {
        ...
        handler.post(() -> {
            if (runningSession != null && !runningSession.isDone()) return;
            ProbeRequest next;
            synchronized (queue) {
                next = queue.poll();
            }
        ...
        try {
            BluetoothGattCallback callback = new BluetoothGattCallback() {
                ...
                public void onCharacteristicChanged(BluetoothGatt g,
                                                    BluetoothGattCharacteristic characteristic) {
                    if (session.isDone() || characteristic == null) return;
                    byte[] data = characteristic.getValue();
                    if (data != null && data.length > 0
                            && (data[0] & 0xFF) == (RESP_SELF_INFO & 0xFF)) {
